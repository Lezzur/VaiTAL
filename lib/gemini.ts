...
export async function extractDataFromText(textInput: string): Promise<ExtractedCheckup> {
  const prompt = `
    You are an expert medical data analyst. Your task is to extract structured lab results from the provided raw text of a medical report.
    The text may be unstructured, pasted from a PDF, or manually typed.
